...
class OpenAI_TTS(BaseTTS):
    ...
    def _validate(self):
        ...
        if 'voice' not in tts_config:
            ...
            self.voice:Literal['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer'] = tts_config['voice'].lower()
    ...
    def __generate_audio(self, text):
        try:
            ...
            client = OpenAI()
            with  client.audio.speech.with_streaming_response.create(model=self.model,voice=self.voice,input=text) as response:
